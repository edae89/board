		...
		<table>
			...
			<c:forEach var="cList" items="${commentList}">
				<tr >
					<td>글번호 : ${cList.num}</td>
					<td>작성자 : ${cList.id}</td>
					<td>댓글 : ${cList.cmemo}</td>
					<td>댓글번호 : ${cList.cnum}</td>
					<td><fmt:formatDate value="${cList.cdate }" /></td>
				</tr>
			</c:forEach>
			<tr>
			...
			<tr >
					<form action="BoardServlet?command=board_comment&cid=${sessionScope.userid }" method="post">
					<td>작성자 : ${sessionScope.userid }</td>
					<td colspan=2>내용 :  <input type=text name="commentmemo" size=100></td>
					<input type=hidden name="commentnum" value="${board.num}">
